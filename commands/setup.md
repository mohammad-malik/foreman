...
!`node "${CLAUDE_PLUGIN_ROOT}/scripts/external-agents.mjs" doctor`

Read the report and tell the user what to do next, shortest path first. Common cases:

- No workspaces registered: give them the `/external-agents:register <path>` line for the repo they are in.
