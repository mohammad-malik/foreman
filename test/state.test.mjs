...
test("EXTERNAL_AGENTS_STATE_DIR wins over everything", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "ea-state-"));
  const result = withEnv(
    { EXTERNAL_AGENTS_STATE_DIR: dir, CLAUDE_PLUGIN_DATA: path.join(dir, "external-agents") },
    resolveStateRoot
  );

  assert.equal(result.root, path.resolve(dir));
  assert.equal(result.source, "EXTERNAL_AGENTS_STATE_DIR");
});

test("CLAUDE_PLUGIN_DATA is used when it names this plugin", () => {
  const dir = path.join(os.tmpdir(), "plugins", "data", "external-agents");
...
test("CLAUDE_PLUGIN_DATA belonging to another plugin is rejected, not used", () => {
  ...
  const localAppData = fs.mkdtempSync(path.join(os.tmpdir(), "ea-lad-"));
  ...
  assert.equal(result.root, path.join(path.resolve(localAppData), "external-agents"));
...
test("falls back to XDG_DATA_HOME when LOCALAPPDATA is absent", () => {
  const xdg = fs.mkdtempSync(path.join(os.tmpdir(), "ea-xdg-"));
  const result = withEnv({ XDG_DATA_HOME: xdg }, resolveStateRoot);

  assert.equal(result.source, "XDG_DATA_HOME");
  assert.equal(result.root, path.join(path.resolve(xdg), "external-agents"));
});
